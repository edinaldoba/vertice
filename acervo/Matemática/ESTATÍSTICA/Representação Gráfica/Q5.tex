...
\begin{tikzpicture}[>=latex, xscale=0.30, yscale=0.45]
  ...
  \draw[fill=VinhoDestaque!70] (0,0.5) rectangle node[right=1.500cm] {\bf\scriptsize 10} (10,1.5);
  \draw[fill=LaranjaAlerta!70] (0,2.0) rectangle node[right=0.300cm] {\bf\scriptsize 2} (2,3.0);
  \draw[fill=VerdeEscola!70]   (0,3.5) rectangle node[right=0.450cm] {\bf\scriptsize 3} (3,4.5);
  \node[left] at (0,1) {\scriptsize fácil};
  \node[left] at (0,2.5) {\scriptsize mediana};
  \node[left] at (0,4) {\scriptsize difícil};
\end{tikzpicture}

Se essa classificação fosse apresentada em um gráfico de setores, o ângulo do setor referente à categoria ``\textbf{difícil}'' mediria:
